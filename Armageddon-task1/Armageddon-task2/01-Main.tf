...
# resource "google_compute_subnetwork" "armageddon2_subnet" {
#   name          = "vanguard64-subnet"
#   ip_cidr_range = "10.0.0.0/16"
#   region        = "us-east1"
#   network       = google_compute_network.armageddon2_vpc.id
# }


# add compute instance to the VPC
resource "google_compute_instance" "armageddon2" {
  ...
  # metadata = {
  #   startup-script-url = "${file("scripts/startup.sh")}"
  # }


  metadata_startup_script = file("startup.sh")

  depends_on = [google_compute_network.armageddon2_vpc,
    google_compute_subnetwork.armageddon2_subnet,
